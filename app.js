...
function getBubbleTemplate(type, hue, colorHex) {
    ...
    if (type === 'silver') {
        ...
        // 1. 白銀バブルの気品ある光彩（濁らせず上品なルミナスグローをプラス）
        const glowGrad = ctx.createRadialGradient(0, 0, drawRadius * 0.70, 0, 0, drawRadius * 1.48);
        glowGrad.addColorStop(0, 'rgba(255, 255, 255, 0.38)');
        glowGrad.addColorStop(0.5, 'rgba(226, 232, 240, 0.16)');
        glowGrad.addColorStop(1, 'rgba(226, 232, 240, 0)');
        ctx.fillStyle = glowGrad;
        ctx.beginPath();
        ctx.arc(0, 0, drawRadius * 1.48, 0, Math.PI * 2);
        ctx.fill();
        ...
        // 1. 引き締まった鮮やかな光彩（オーロラ光彩に寄り添う輝き）
        const glowGrad = ctx.createRadialGradient(0, 0, drawRadius * 0.85, 0, 0, drawRadius * 1.20);
        glowGrad.addColorStop(0, `hsla(${hue}, 92%, 68%, 0.28)`);
        glowGrad.addColorStop(1, `hsla(${hue}, 92%, 68%, 0)`);
        ctx.fillStyle = glowGrad;
        ctx.beginPath();
        ctx.arc(0, 0, drawRadius * 1.20, 0, Math.PI * 2);
        ctx.fill();
        
        // 2. Body（彩度を高め、クリスタルのように色鮮やかな球体）
        const bodyGrad = ctx.createRadialGradient(-drawRadius * 0.24, -drawRadius * 0.24, drawRadius * 0.06, 0, 0, drawRadius);
        bodyGrad.addColorStop(0, `hsla(${hue}, 90%, 86%, 0.88)`); // 光の透過部
        bodyGrad.addColorStop(0.35, `hsla(${hue}, 88%, 70%, 0.45)`);
        bodyGrad.addColorStop(0.72, `hsla(${hue}, 85%, 58%, 0.38)`); // 色の深み
        bodyGrad.addColorStop(0.90, `hsla(${hue}, 90%, 62%, 0.72)`); // フレネル反射部
        bodyGrad.addColorStop(1.0, `hsla(${hue}, 92%, 66%, 0.90)`); // 外縁の鮮やかな輪郭
        ctx.fillStyle = bodyGrad;
        ctx.beginPath();
        ctx.arc(0, 0, drawRadius, 0, Math.PI * 2);
        ctx.fill();
        ...
        // 3. クッキリした鮮明な光の輪郭線
        ctx.strokeStyle = `hsla(${hue}, 92%, 82%, 0.88)`;
        ctx.lineWidth = 1.6;
        ctx.beginPath();
        ...
        const hlR = drawRadius * 0.24;
        const hlGrad = ctx.createRadialGradient(hlX, hlY, 0, hlX, hlY, hlR);
        hlGrad.addColorStop(0, 'rgba(255, 255, 255, 0.96)');
        hlGrad.addColorStop(0.45, `hsla(${hue}, 80%, 94%, 0.65)`);
        hlGrad.addColorStop(1, 'rgba(255, 255, 255, 0)');
        ctx.fillStyle = hlGrad;
        ctx.beginPath();
        ctx.arc(hlX, hlY, hlR, 0, Math.PI * 2);
        ...
        const subHlR = drawRadius * 0.18;
        const subHlGrad = ctx.createRadialGradient(subHlX, subHlY, 0, subHlX, subHlY, subHlR);
        subHlGrad.addColorStop(0, `hsla(${hue}, 95%, 88%, 0.50)`);
        subHlGrad.addColorStop(1, `hsla(${hue}, 95%, 88%, 0)`);
        ctx.fillStyle = subHlGrad;
        ctx.beginPath();
        ctx.arc(subHlX, subHlY, subHlR, 0, Math.PI * 2);
        ctx.fill();
...
function drawAuroraParticles(scale, brightnessMultiplier = 1.0) {
    ...
    for (let p of auroraParticles) {
        ...
        const auroraParticleAlphaMod = (IS_MOBILE ? 0.90 : 1.08) * brightnessMultiplier;
...
function drawRealAuroraCurtain(brightnessMultiplier = 1.0) {
    ...
    if (auroraSkipCount >= 3) {
        ...
        for (let ox = 0; ox < offWidth; ox += step) {
            ...
            const baseAuroraAlpha = (IS_MOBILE ? 0.104 : 0.128) * brightnessMultiplier;
...
function createBigExplosionMeteor(hue, originX, originY) {
    ...
    meteors.push({
        ...
        alpha: 0,
        fadeSpeed: 0.45,
        // 大爆発流星の輝度を引き上げ (モバイル 0.44->0.72, PC 0.623->0.88)
        targetAlpha: (0.9 + Math.random() * 0.1) * (IS_MOBILE ? 0.72 : 0.88),
        sparkleChance: 0.8,
        life: 0,
